...
public class RGBHistogramManipulator : Gtk.DrawingArea {
    ...
    private Gdk.Pixbuf? drag_nub_pixbuf = null;

    private Gdk.Pixbuf? get_drag_nub_pixbuf () {
        if (drag_nub_pixbuf == null) {
            try {
                drag_nub_pixbuf = new Gdk.Pixbuf.from_resource ("/org/pantheon/photos/icons/drag_nub.png");  
            } catch (Error err) {
    ...
    private void draw_nub (Cairo.Context ctx, Gdk.Rectangle area, int position) {
        Gdk.cairo_set_source_pixbuf (ctx, get_drag_nub_pixbuf (), area.x + position, area.y + NUB_V_POSITION);
        ctx.paint ();
    }
